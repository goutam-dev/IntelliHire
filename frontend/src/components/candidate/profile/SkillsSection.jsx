...
const SkillsSection = ({ profile, onUpdate, onUnsavedChanges }) => {
  ...
  return (
    <div className="p-6">
      <h2 className="text-xl font-semibold text-zinc-900 mb-6">Skills</h2>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          {skills.length === 0 ? (
            <div className="text-center py-8 bg-zinc-50 rounded-2xl border border-zinc-200">
              ...
              <h4 className="text-lg font-medium text-zinc-900 mb-2">No Skills Added</h4>
              <p className="text-zinc-600">Add your skills to help employers find you</p>
            </div>
          ) : (
            <div className="flex flex-wrap gap-2">
              {skills.map((skill, index) => (
                <span
                  key={index}
                  className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 border border-zinc-300"
                >
                  {skill}
                  <button
                    onClick={() => removeSkill(skill)}
                    className="ml-2 inline-flex items-center justify-center w-4 h-4 rounded-full text-zinc-900 hover:bg-blue-200 hover:text-blue-800 transition-colors"
                    title="Remove skill"
                  >
          ...
          <div className="relative">
            ...
            {inputValue && (
              <button
                ...
                className="absolute right-2 top-1/2 transform -translate-y-1/2 px-3 py-1 text-sm bg-zinc-900 text-white rounded hover:bg-zinc-800 transition-colors"
              >
                Add
              </button>
            )}
            ...
            <div className="flex flex-wrap gap-2">
              {filteredSuggestions.map((skill) => (
                <button
                  ...
                  className="px-3 py-1 text-sm border border-zinc-300 rounded-full hover:bg-zinc-50 hover:border-zinc-400 transition-colors"
                >
                  + {skill}
                </button>
              ))}
        ...
        {skills.length === 0 && !inputValue && (
          <div className="bg-zinc-50 border border-zinc-300 rounded-2xl p-4">
            <h4 className="text-sm font-medium text-blue-800 mb-2">Skill Categories</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-zinc-800">
              <div>
